...
fn parseNextNumber(context: *ParserContext) ?ListEntry {
    const slice_len = for (context.remaining, 0..) |char, index| {
...
fn parseStrComplete(input_str: []const u8, allocator: Allocator) ListEntry {
    var context = ParserContext{ .allocator = allocator, .remaining = input_str };
