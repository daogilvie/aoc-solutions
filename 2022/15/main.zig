...
pub fn solve(content: str, allocator: Allocator, target_y: isize, search_bound: isize) !Answer {
    ...
    for (sensors.items, 0..) |sensor_lhs, lhs_index| {
        const lhs_distance = sensor_distances.items[lhs_index];
        for (sensors.items, 0..) |sensor_rhs, rhs_index| {
    ...
    const experimental: usize = exp: while (bp_iter.next()) |entry| {
        ...
        inner: for (sensors.items, 0..) |sensor, index| {
...
pub fn main() !void {
    ...
    var answer = solve(input, allocator, 2000000, 4000000) catch unreachable;
    answer.print();
}
