...
const BunchOfStacks = struct {
    ...
    fn getTopRow(self: BunchOfStacks) []u8 {
        ...
        for (self.stacks.items, 0..) |stack, index| {
...
fn parseStartingStacks(stack_definitions: []const u8, allocator: Allocator) !BunchOfStacks {
    var stack_lines_reordered = std.mem.splitBackwards(u8, stack_definitions, "\n");
    ...
    while (stack_lines_reordered.next()) |layer| {
        for (bunch_of_stacks.stacks.items, 0..) |*stack, index| {
...
pub fn solve(contents: []const u8, allocator: Allocator) !Answer {
